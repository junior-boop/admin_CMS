export function createCachedCollectionClient(db, cache, tableName, _definition) {
    ...
    const cacheKey = (id) => cache.getCacheKey(tableName, id?.toString());
    return {
        async find(options = {}) {
            const { where, limit = 100, offset = 0, orderBy } = options;
            const cacheData = await cache.getOrFetch(cacheKey(), async () => {
        ...
        async findOne(id) {
            const cacheData = await cache.getOrFetch(cacheKey(id), async () => {
...
//# sourceMappingURL=cached-collections.js.map