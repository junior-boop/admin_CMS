export type FieldType = 'text' | 'richtext' | 'number' | 'boolean' | 'date' | 'select' | 'media' | 'relation';
interface BaseField {
    type: FieldType;
    required?: boolean;
    label?: string;
...
export type Field = TextField | RichtextField | NumberField | BooleanField | DateField | SelectField | MediaField | RelationField;
...
type InferFieldType<F extends Field> = F extends TextField ? string : F extends RichtextField ? string : F extends NumberField ? number : F extends BooleanField ? boolean : F extends DateField ? string : F extends SelectField ? F['options'][number] : F extends MediaField ? {
    key: string;
    url: string;
} : F extends RelationField ? string : never;
